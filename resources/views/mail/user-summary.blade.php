...
@if($document_review_data)
{{
    sprintf(
        __('You have %s document(s) you need to review.'),
        count($document_review_data['documentIds'])
    )
}}
<x-mail::button
    :url="route('documents.index', ['filters[ids]' => implode(', ', $document_review_data['documentIds'])])"
>
    {{__('View documents')}}
</x-mail::button>
@endif
