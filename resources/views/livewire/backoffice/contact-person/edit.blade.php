<div>
    <x-form-header backlink="{{ route('backoffice.companies.show', $company) }}">
        <x-slot name="title">{{ __('Edit Contact Person') }}</x-slot>
    </x-form-header>
    <form
        class="bottom-0 right-0 px-4 pt-8 pb-4 leading-6 border-b-0 border-gray-200 border-solid text-blackborder-t border-x-0"
        wire:submit="update"
    >
        @include('livewire.backoffice.contact-person.form')
