<div>
    ...
    <div class="flex flex-col leading-6 text-black">
        ...
        <div class="overflow-x-auto">
            <x-table>
                <x-slot name="head">
                    @foreach($columns as $column)
                        ...
                        @if(
                            App\Models\WeldingCoordination::SYSTEM_COLUMNS[$column->key]['type'] == 'relationship' ||
                            App\Models\WeldingCoordination::SYSTEM_COLUMNS[$column->key]['type'] == 'calculated'
                        )
                            <x-table.heading wire:click="sortBy('{{ $column->key }}' )" sortable :multiColumn="false" :direction="$sorts['{{ $column->key }}'] ?? null">{{ App\Models\WeldingCoordination::SYSTEM_COLUMNS[$column->key]['label'] }}</x-table.heading>
                        @else
                            <x-table.heading wire:click="sortBy('data->{{ $column->key }}')" sortable :multiColumn="false" :direction="$sorts['data->{{ $column->key }}'] ?? null">{{ App\Models\WeldingCoordination::SYSTEM_COLUMNS[$column->key]['label'] }}</x-table.heading>
                        @endif
                    @endforeach
                    <x-table.heading />
                </x-slot>
                <x-slot name="body">
                    @foreach($weldingCoordinations as $weldingCoordination)
                        <x-table.row
                        ...
                        >
                            @foreach($columns as $column)
                                ...
                                <x-table.model-value-cell
                                    :key="$column->key"
                                    :column="App\Models\WeldingCoordination::SYSTEM_COLUMNS[$column->key]"
                                    :model="$weldingCoordination"
                                />
                            @endforeach

                            <x-table.cell class="text-right">
                                <div class="flex">
                                    @can('update', $weldingCoordination)
                                        <x-button.link
                                            href="{{ route('welding-coordination.edit', $weldingCoordination) }}"
                                            class="text-gray-600 bg-transparent hover:bg-gray-100 hover:text-gray-900"
                                        >
                                            <x-icon.pencil class="w-4 h-4 text-gray-800" />
                                        </x-button.link>
                                    @endcan
                                    @can('delete', $weldingCoordination)
                                        <div class="flex" x-data @click.prevent.stop="console.log('stop')">
                                            @if($confirming == $weldingCoordination->id)
                                                <x-button
                                                    wire:click="delete({{ $weldingCoordination->id }})"
                                            ...
                                            @else
                                                <x-button
                                                    wire:click="confirmDelete({{ $weldingCoordination->id }})"
            ...
            @if ($weldingCoordinations->hasPages())
                <div class="px-6 py-4 bg-white border-t">
                    {{ $weldingCoordinations->links() }}
                </div>
            @endif
        </div>
    </div>
