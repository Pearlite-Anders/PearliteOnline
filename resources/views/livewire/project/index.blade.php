<div>
    ...
    <x-index-header>
        <x-slot name="heading">
            <x-icon.project class="w-6 h-6 mr-2 text-gray-500 align-middle duration-75 ease-in-out" />
            {{ __('Project') }}
        </x-slot>
        <x-slot name="search">
        ...
        <x-slot name="buttons">
            ...
            @can('create', App\Models\Project::class)
                <x-button.link href="{{ route('project.create') }}" class="inline-flex items-center justify-center whitespace-nowrap">
            ...
            <x-table>
                <x-slot name="head">
                    @foreach($columns as $column)
                        ...
                        @if(
                            App\Models\Project::SYSTEM_COLUMNS[$column->key]['type'] == 'relationship' ||
                            App\Models\Project::SYSTEM_COLUMNS[$column->key]['type'] == 'calculated'
                        )
                            <x-table.heading wire:click="sortBy('{{ $column->key }}' )" sortable :multiColumn="false" :direction="$sorts['{{ $column->key }}'] ?? null">{{ App\Models\Project::SYSTEM_COLUMNS[$column->key]['label'] }}</x-table.heading>
                        @else
                            <x-table.heading wire:click="sortBy('data->{{ $column->key }}')" sortable :multiColumn="false" :direction="$sorts['data->{{ $column->key }}'] ?? null">{{ App\Models\Project::SYSTEM_COLUMNS[$column->key]['label'] }}</x-table.heading>
                        @endif
                    @endforeach
                    <x-table.heading />
                </x-slot>
                        ...
                        >
                            @foreach($columns as $column)
                                ...
                                <x-table.model-value-cell
                                    :key="$column->key"
                                    :column="App\Models\Project::SYSTEM_COLUMNS[$column->key]"
                                    :model="$project"
                                />
                            @endforeach
