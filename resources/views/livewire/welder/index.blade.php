<div>
    ...
    <x-index-header :compressed_header="$compressed_header">
        <x-slot name="heading">
            <x-icon.welder class="w-6 h-6 mr-2 text-gray-500 align-middle duration-75 ease-in-out" />
            {{ __('Welder') }}
        </x-slot>
        <x-slot name="search">
        ...
        <x-slot name="buttons">
            ...
            @can('create', App\Models\Welder::class)
                <x-button.link href="{{ route('welder.create') . ( $project_id ? '?project_id='. $project_id : '') }}" class="inline-flex items-center justify-center whitespace-nowrap">
            ...
            <x-table>
                <x-slot name="head">
                    @foreach($columns as $column)
                        ...
                        @if(
                            App\Models\Welder::SYSTEM_COLUMNS[$column->key]['type'] == 'relationship' ||
                            App\Models\Welder::SYSTEM_COLUMNS[$column->key]['type'] == 'calculated'
                        )
                            <x-table.heading wire:click="sortBy('{{ $column->key }}' )" sortable :multiColumn="false" :direction="$sorts['{{ $column->key }}'] ?? null">{{ App\Models\Welder::SYSTEM_COLUMNS[$column->key]['label'] }}</x-table.heading>
                        @else
                            <x-table.heading wire:click="sortBy('data->{{ $column->key }}')" sortable :multiColumn="false" :direction="$sorts['data->{{ $column->key }}'] ?? null">{{ App\Models\Welder::SYSTEM_COLUMNS[$column->key]['label'] }}</x-table.heading>
                        @endif
                    @endforeach
                    <x-table.heading />
                </x-slot>
                        ...
                        >
                            @foreach($columns as $column)
                                ...
                                <x-table.model-value-cell
                                    :key="$column->key"
                                    :column="App\Models\Welder::SYSTEM_COLUMNS[$column->key]"
                                    :model="$welder"
                                />
                            @endforeach
                                ...
                                <div class="flex">
                                    ...
                                    @can('delete', $welder)
                                        <div class="flex" x-data @click.prevent.stop="console.log('stop')">
                                            @if($confirming == $welder->id)
                                                <x-button
                                                    wire:click="delete({{ $welder->id }})"
                                            ...
                                            @else
                                                <x-button
                                                    wire:click="confirmDelete({{ $welder->id }})"
