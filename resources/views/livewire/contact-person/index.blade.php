<div>
    ...
    <x-index-header>
        ...
        <x-slot name="buttons">
            ...
            @can('create', App\Models\ContactPerson::class)
                <x-button.link href="{{ route('contact-person.create', $company) }}" class="inline-flex items-center justify-center whitespace-nowrap">
            ...
            <x-table>
                <x-slot name="head">
                    @foreach($columns as $column)
                        ...
                        @if(
                            App\Models\ContactPerson::SYSTEM_COLUMNS[$column->key]['type'] == 'relationship' ||
                            App\Models\ContactPerson::SYSTEM_COLUMNS[$column->key]['type'] == 'calculated'
                        )
                            <x-table.heading wire:click="sortBy('{{ $column->key }}' )" sortable :multiColumn="false" :direction="$sorts['{{ $column->key }}'] ?? null">{{ App\Models\ContactPerson::SYSTEM_COLUMNS[$column->key]['label'] }}</x-table.heading>
                        @else
                            <x-table.heading wire:click="sortBy('data->{{ $column->key }}')" sortable :multiColumn="false" :direction="$sorts['data->{{ $column->key }}'] ?? null">{{ App\Models\ContactPerson::SYSTEM_COLUMNS[$column->key]['label'] }}</x-table.heading>
                        @endif
                    @endforeach
                    <x-table.heading />
                </x-slot>
                <x-slot name="body">
                    @foreach($contactPersons as $contactPerson)
                        <x-table.row
                            :edit_link="route('contact-person.edit', [$company, $contactPerson])"
                        ...
                        >
                            @foreach($columns as $column)
                                ...
                                <x-table.model-value-cell
                                    :key="$column->key"
                                    :column="App\Models\ContactPerson::SYSTEM_COLUMNS[$column->key]"
                                    :model="$contactPerson"
                                />
                            @endforeach
                                ...
                                <div class="flex">
                                    ...
                                    @can('delete', $contactPerson)
                                        <div class="flex" x-data @click.prevent.stop="console.log('stop')">
                                            ...
                                            @else
                                                <x-button
                                                    wire:click="confirmDelete({{ $contactPerson->id }})"
